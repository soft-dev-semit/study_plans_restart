...
public class SpecialtyService {
  ...
  public Specialities specialtyById(Long specialty_id) {
    return specialtyRepository
        .findById(specialty_id)
        .orElseThrow(() -> new RuntimeException("wrong specialty_id"));
  }
  ...
  public Specialities create(CreateSpecialtyDTO specialtyDto) {
    Optional<Specialities> specialty =
        specialtyRepository.findByCodeAndName(specialtyDto.getCode(), specialtyDto.getName());
    return specialty.orElseGet(
        () -> specialtyRepository.save(modelMapper.map(specialtyDto, Specialities.class)));
  }
