...
public class PlansNPCreate {
  ...
  public static Sheet createHeader(Workbook workbook) {
    ...
    headerRow
        .createCell(12)
        .setCellValue("Розподіл аудиторних годин на тиждень та кредитів ECTS за семестрами");
  ...
  public void fillCell(Sheet sheet, long curriculum_id) {
    ...
    long ZPcount =
        plansInfo.stream()
            .filter(
                d -> d.getDiscipline() != null && d.getDiscipline().getShortName().startsWith("ЗП"))
            .count();
    long VDcount =
        plansInfo.stream()
            .filter(
                d -> d.getDiscipline() != null && d.getDiscipline().getShortName().startsWith("ВД"))
            .count();
    long SPcount =
        plansInfo.stream()
            .dropWhile(
                d ->
                    d.getDiscipline() == null
                        || !"Спеціальна (фахова) підготовка".equals(d.getDiscipline().getName()))
            .takeWhile(
                d -> d.getDiscipline() == null || !"Атестація*".equals(d.getDiscipline().getName()))
            .count();
    long count =
        plansInfo.stream()
            .dropWhile(
                d ->
                    d.getDiscipline() == null
                        || !"Загальна підготовка".equals(d.getDiscipline().getName()))
            .takeWhile(
                d ->
                    d.getDiscipline() == null
                        || !"Спеціальна (фахова) підготовка".equals(d.getDiscipline().getName()))
    ...
    long number =
        plansInfo.stream()
            .dropWhile(
                d ->
                    d.getDiscipline() == null
                        || !"Профільна підготовка".equals(d.getDiscipline().getName()))
            .takeWhile(
                d ->
                    d.getDiscipline() == null
                        || !"Дисципліни вільного вибору студента профільної підготовки згідно переліку"
                            .equals(d.getDiscipline().getName()))
            .count();
    long RD1count =
        plansInfo.stream()
            .dropWhile(
                d ->
                    d.getDiscipline() == null
                        || !d.getDiscipline()
                            .getName()
                            .startsWith("Профільований пакет дисциплін 01"))
            .takeWhile(
                d ->
                    d.getDiscipline() == null
                        || !d.getDiscipline()
                            .getName()
                            .startsWith("Профільований пакет дисциплін 02"))
            .count();
    long RD2count =
        plansInfo.stream()
            .dropWhile(
            ...
            .takeWhile(
                d ->
                    d.getDiscipline() == null
                        || !d.getDiscipline()
                            .getName()
                            .startsWith("Профільований пакет дисциплін 03"))
            .count();
    long RD3count =
        plansInfo.stream()
            .dropWhile(
    ...
    for (PlansRow list : plansInfo) {
      ...
      if ("Спеціальна (фахова) підготовка".equals(list.getDiscipline().getName())
          || "Загальна підготовка".equals(list.getDiscipline().getName())) {
        row.createCell(0).setCellValue(list.getDiscipline().getShortName());
        row.createCell(1).setCellValue(list.getDiscipline().getName());
      ...
      if (list.getDiscipline().getName().startsWith("Профільований пакет дисциплін 02")
          || list.getDiscipline().getName().startsWith("Профільований пакет дисциплін 03")) {
        row.createCell(0).setCellValue(list.getDiscipline().getShortName());
        row.createCell(1).setCellValue(list.getDiscipline().getName());
  ...
  public static void fillSemesterCell(Row row, PlansRow list) {
    ...
    for (SemesterDTO semester : list.getSemesters()) {
      ...
      row.createCell(12 + (semester.getSemester() - 1) * 2 + 1)
          .setCellValue(semester.getCreditsECTS());
