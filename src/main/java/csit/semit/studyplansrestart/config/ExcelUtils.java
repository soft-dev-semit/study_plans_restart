...
public class ExcelUtils {
      ...
      if (cell.getCellType() == CellType.STRING) {
        ...
        if (cellValue.contains("-")) {
          ...
          try {
            ...
            log.warn(
                "Failed to parse credits from string: '{}', parts: {}, at row: {}",
                cellValue,
  ...
  public static void addFormulaPlansCell(int rowNum, Row row) {
    row.createCell(5)
        .setCellFormula(
            "N" + rowNum + "+P" + rowNum + "+R" + rowNum + "+T" + rowNum + "+V" + rowNum + "+X"
                + rowNum + "+Z" + rowNum + "+AB" + rowNum);
  ...
  public static void fillProfilePack(Row row, int from, int to, int RDrow) {
    row.createCell(5)
        .setCellFormula(
            "IF(SUM(F" + from + ":F" + to + ")=F$" + RDrow + ",F$" + RDrow + ",\"ОШИБКА\")");
    row.createCell(6)
        .setCellFormula(
            "IF(SUM(G" + from + ":G" + to + ")=G$" + RDrow + ",G$" + RDrow + ",\"ОШИБКА\")");
    row.createCell(7)
        .setCellFormula(
            "IF(SUM(H" + from + ":H" + to + ")=H$" + RDrow + ",H$" + RDrow + ",\"ОШИБКА\")");
    row.createCell(8)
        .setCellFormula(
            "IF(SUM(I" + from + ":I" + to + ")=I$" + RDrow + ",I$" + RDrow + ",\"ОШИБКА\")");
    row.createCell(9)
        .setCellFormula(
            "IF(SUM(J" + from + ":J" + to + ")=J$" + RDrow + ",J$" + RDrow + ",\"ОШИБКА\")");
    row.createCell(10)
        .setCellFormula(
            "IF(SUM(K" + from + ":K" + to + ")=K$" + RDrow + ",K$" + RDrow + ",\"ОШИБКА\")");
    row.createCell(11)
        .setCellFormula(
            "IF(SUM(L" + from + ":L" + to + ")=L$" + RDrow + ",L$" + RDrow + ",\"ОШИБКА\")");
    row.createCell(16)
        .setCellFormula(
            "IF(SUM(Q" + from + ":Q" + to + ")=Q$" + RDrow + ",Q$" + RDrow + ",\"ОШИБКА\")");
    row.createCell(17)
        .setCellFormula(
            "IF(SUM(R" + from + ":R" + to + ")=R$" + RDrow + ",R$" + RDrow + ",\"ОШИБКА\")");
    row.createCell(18)
        .setCellFormula(
            "IF(SUM(S" + from + ":S" + to + ")=S$" + RDrow + ",S$" + RDrow + ",\"ОШИБКА\")");
    row.createCell(19)
        .setCellFormula(
            "IF(SUM(T" + from + ":T" + to + ")=T$" + RDrow + ",T$" + RDrow + ",\"ОШИБКА\")");
    row.createCell(20)
        .setCellFormula(
            "IF(SUM(U" + from + ":U" + to + ")=U$" + RDrow + ",U$" + RDrow + ",\"ОШИБКА\")");
    row.createCell(21)
        .setCellFormula(
            "IF(SUM(V" + from + ":V" + to + ")=V$" + RDrow + ",V$" + RDrow + ",\"ОШИБКА\")");
    row.createCell(22)
        .setCellFormula(
            "IF(SUM(W" + from + ":W" + to + ")=W$" + RDrow + ",W$" + RDrow + ",\"ОШИБКА\")");
    row.createCell(23)
        .setCellFormula(
            "IF(SUM(X" + from + ":X" + to + ")=X$" + RDrow + ",X$" + RDrow + ",\"ОШИБКА\")");
    row.createCell(24)
        .setCellFormula(
            "IF(SUM(Y" + from + ":Y" + to + ")=Y$" + RDrow + ",Y$" + RDrow + ",\"ОШИБКА\")");
    row.createCell(25)
        .setCellFormula(
            "IF(SUM(Z" + from + ":Z" + to + ")=Z$" + RDrow + ",Z$" + RDrow + ",\"ОШИБКА\")");
  }
  ...
  public static void fillLastRow(Sheet sheet, int index, int M22, int secondComponent) {
    ...
    List<String> stringList =
        Arrays.asList(
            "Кількість екзаменів", "Кількість заліків", "Кількість курсових проектів (робіт)");
    ...
    // String formula = "=COUNT(M14:M"+ M22 +")+COUNT(M"+(M22 + 2) +":M"+(secondComponent -1)
    // +")+COUNT(M"+(secondComponent + 3) +":M"+(secondComponent + 8) +")+COUNT(M" + (index -1)
    // +":M"+(index +1) +")+COUNT(M"+(index -3) +")";
